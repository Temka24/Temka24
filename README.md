# 👋 Hi, I'm Temuujin

I'm a self taught fullstack developer and SaaS Founder.
I working on build real world products that solve real problems.

Currently building [TimeLink](https://timelink.mn) — a booking SaaS that helps individuals and businesses manage appointments easily automation.
and my second SaaS Lerno - a more English word memorizing flashcard app that to helps people more faster to learn english.

---

🚀 Fullstack Developer | SaaS Founder
🎯 On a mission to build software that save people time and money

---

### 🚀 What I'm Working On

- Building and scaling **TimeLink.mn** – Time slot booking platform
- Learning more about infrastructure, scalability & system design


### 💼 Projects

- [timelink.mn](https://timelink.mn) – My First SaaS
- Lerno - My Second Project to help people memorizing English words
- [me.timelink.mn](https://me.timelink.mn) – My portfolio site
- [shopaura](https://shopaura-gray.vercel.app) – Full featured E-commerce store
- [chat-app](https://real-time-chat-app-five-dun.vercel.app) – Real-time chat app with stunning UI with socket.io

---

### 🛠️ Tech Stack

#### Languages 
- `JavaScript`, `TypeScript`, `Python`, `C++`,

### Frontend 
- `React.js`, `Next.js`, `TailwindCSS`, `Shadcn UI`, `React-Query`, `Zustand`, `Redux ToolKit`,

 `NestJS`, `PostgreSQL`, `Prisma`, `WebSocket`, `FastAPI`, `Turborepo`, `Inngest`, `Upstash`, `Monitoring`, `Analyst`, `Expo`, ``, `System Design`,

---

### 📫 Contacts

- Email: temuujin.buy@gmail.com
- Portfolio: [me.timelink.mn](https://me.timelink.mn)
- LinkedIn: [https://www.linkedin.com/in/temuujin-b-672929322/](#)

